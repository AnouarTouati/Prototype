...
public class GraphOverlay : MonoBehaviour
{
	...
	public float longData;
	public float latData;
	void Start()
	{
	...
	void FixedUpdate()
	{
		if (vehicleBody == null)
		{
			if (GameObject.FindGameObjectWithTag("Player")) {
				vehicleBody = GameObject.FindGameObjectWithTag("Player").GetComponent<Rigidbody>();
				SetupWheelConfigs();
			}
		}
		foreach (var wheelConfig in wheelConfigs)
		{
            ...
            if (Mathf.Abs(timeTravel) < k_MaxRecordTimeTravel)// TODO: solve this mystery
			{
				...
				longData = hit.forwardSlip;
				latData = hit.sidewaysSlip;
			}
		}
	}
